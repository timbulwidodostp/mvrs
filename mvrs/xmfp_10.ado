...
while !`stable' & `it'< `cycles' {
	...
	while `i'<=`nx' {
		...
		local ni `n`r''
		local dfi df(`df`r'')
// noi di in red "START  PROCESS: i=`i' ri=`r' ni=`ni' dfr=`df`r'' hr=`h`r'' por=`po`r''"
/*
...
// noi di in red "END OF PROCESS: i=`i' ri=`r' ni=`ni' dfr=`df`r'' hr=`h`r'' por=`po`r''" _n "{hline 79}"
		local i = `i'+1
	}
	if `lastch'==1 {
		local stable 1
...
local xvar `n'
// noi di in red `"xfrac_154 `cmd' `lhs' `n' `fixpowers' `nn' `base' `if' `in' `weight', `degree' `powers' `options' `catzero'"'
`VV' ///
...
/*
	...
	Residual df for models 1 and 3 updated 05may2017.
	!! NEEDS CHECKING cf code near line 1091 - n2 for given largest model should be constant.
*/
version 11
gettoken cmd 0 : 0
...
quietly {
	...
	/*
		Note that `rdf' is the residual df EXCLUDING FP terms.
		`x1' is the only FP-related variable here.
		This is confusing terminology! Care is needed in handling the
		correct residual df for FP models.
	*/
	local rdf = e(fp_rdf)
	...
	// Get deviance for optimal (FP1, FP1A) model (model 1)
	local p1b 
	local p2b
...
local n1_0 = (`k' + 1) * 2 // model df for highest-dimension model (FP1 + FP1A) vs null
local n2 = `rdf' - `n1_0'
